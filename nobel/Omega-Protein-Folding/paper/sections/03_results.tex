...
\textbf{Blind-sprint feasibility.} Finally, we tested whether Auric can be used as an \emph{active scoring signal} (not just a post-hoc discriminator). On short targets (N<100), we ran blind-sprint prototypes that do not use oracle direction guidance and treat TM-score as evaluation-only, optimizing native-derived contact/distogram objectives augmented by Auric penalties. For example, on 1CRN a contact+distogram+Auric beam-search run reaches TM$>$0.6 while maintaining good certificate values (see \texttt{docs/runs/blind\_sprint\_auric\_small/}). While these prototypes are not yet the full Omega operator stack, they establish feasibility that Auric-style certificates can steer search toward higher-quality structures.
