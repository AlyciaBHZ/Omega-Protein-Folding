...
We first tested whether macro-operator navigation can be made \textbf{self-adaptive} without model training, using an error-geometry impedance score $\mathrm{Imp}(p;D)$ to select wormhole candidates. In biophysical terms, $\mathrm{Imp}$ plays the role of a landscape \emph{roughness/frustration} proxy derived from residual geometry: it is not a physical energy, but a protocol-fixed scalar field that shapes exploration. On N=16 contact-map proxy tasks across several targets, the impedance-guided selector achieves competitive best Contact\_F1 while exhibiting target-dependent operator usage. In particular, the 2CI2 proxy concentrates selection on \textbf{Expander\_Hot} macro-patches, consistent with the interpretation that its residual is dominated by localized repair needs, whereas 1UBQ uses predominantly local moves and global expander patches.
...
Across this proxy suite, the best mean Contact\_F1 reaches 0.962 on 2CI2\_16 and 5PTI\_16 (Imp-Adaptive-v2) and 0.916 on 1UBQ\_16 (Mix-Expander-Hot), while exhibiting different macro-operator usage patterns (Fig.~\ref{fig:2}; Table~\ref{tab:1}). Operationally, wormholes act like \emph{basin-hopping} moves: structured, auditable nonlocal rewrites that help traverse barriers induced by low-resolution objectives.
...
As an additional low-cost sanity check that does not modify search, we computed coarse-grained C$\alpha$ physical proxies (bond-length MSE and a non-neighbour steric-clash proxy) and compared them against the Auric geometry certificate. On the \texttt{4state\_reduced} hard-negative decoy benchmark, these two signals are weakly correlated (Pearson $r\approx 0$), consistent with an \textbf{orthogonal-defense} interpretation: many decoys satisfy local physical constraints while violating global geometric manifold constraints (high Auric entropy), and a smaller subset exhibits the converse (low Auric entropy but poor local physics). Figure~S\ref{fig:physics_audit} summarizes this ``dual-filter funnel'' behavior.

\textbf{AlphaFold transparency context (non-accuracy).} To contextualize Omega's auditability claim against widely used predictors, we also include a qualitative transparency comparison against \texttt{AlphaFold} DB (AFDB), contrasting a static AFDB output + pLDDT profile with Omega's replayable audit trajectory (Fig.~\ref{fig:af_vs_omega_transparency}; artifacts under \texttt{docs/runs/af\_vs\_omega/}).
...
\textbf{Auric certificates rescue search failure cases.}
