...
As an additional low-cost sanity check that does not modify search, we computed coarse-grained C$\alpha$ physical proxies (bond-length MSE, steric-clash proxy, and $R_g$) and correlated them with the Auric geometry certificate. On a native-vs-decoy toy set, low-$H(\mathrm{type})$ structures tend to avoid extreme steric crowding, consistent with the interpretation that Auric-style geometric feasibility implicitly filters grossly non-physical configurations (Fig.~S\ref{fig:physics_audit}; scripts and CSV under \texttt{docs/runs/physics\_audit/}).
