...
\textbf{Hard-negative decoy control.} To address the reviewer concern that random or shuffle nulls are ``too easy'' (i.e., trivially non-protein-like), we evaluated Auric on a classic hard-negative benchmark: Decoys 'R' Us \texttt{4state\_reduced}, which contains near-native but incorrect folds. Using the same hybrid protocol and a protocol-fixed shared-PCA axis per target (native-derived $u$ reused for all decoys), $\rho_A$ type-entropy typically ranks the native structure among the lower-entropy tail of its decoy set, with several targets showing near-complete separation (e.g., 1R69 and 2CRO). Quantitatively, for $\rho_A$ and $H(\mathrm{type})$ at $m=8$, the native lies within the lowest 30\% of its decoy set for 6/7 targets (and within the lowest 10\% for 2/7 targets). A minority case (1CTF) exhibits weak or reversed separation under this particular decoy family, highlighting that decoys can sometimes preserve (or overemphasize) specific certificate signatures. Full decoy reports and per-target histograms are provided in \texttt{docs/runs/decoys\_4state\_auric/}, and we recommend extending the same pipeline to a second decoy family (e.g. \texttt{lmds}) as a reviewer-facing robustness check.
