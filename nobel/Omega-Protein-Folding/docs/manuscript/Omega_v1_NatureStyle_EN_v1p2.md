...
**Multi-target three-way audit (1R69, 2CRO).** To contextualize this “physics audit” against specialist ab initio predictors, we performed the same constrained FastRelax audit on committable server models for **QUARK** and **I‑TASSER** on representative targets **1R69** and **2CRO** (model1; \(nstruct=20\)). TM-scores in Figure 5A use the repository’s **Kabsch single-pass TM-score** (see Supplementary; `server_summaries.csv`) to provide a consistent topological readout across targets. While QUARK/I‑TASSER achieve higher TM on these server models, the physical audit reveals a complementary advantage: **Omega exhibits a lower drift distribution under Rosetta relaxation across both targets**, with lower median Cα drift to its input trace and a shorter tail than fragment-/template-based baselines (Figure 5B). In addition, the best-scoring relaxed Omega models reach a lower Rosetta energy floor than QUARK and I‑TASSER under the same score function and restraint strength, suggesting that the geometric solver tends to land in basins that are particularly compatible with atomic force-field optimization. Figure 5C visualizes a qualitative Cα overlay against native (1R69).
